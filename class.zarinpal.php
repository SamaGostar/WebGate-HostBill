...
//@prod:    HostBill
//@proj:    Zarinpal Payment Gateway
//@date:    6/3/14
//@time:    12:05 PM
//@path:    /includes/modules/Payment/zarinpal/class.zarinpal.php
//@desc:    N/A

class zarinpal extends PaymentModule
{
    protected $modname = 'Zarinpal Payment Gateway';
    protected $description = 'Zarinpal Payment Gateway<br /> by Masoud Amini <br />';
    ...
    {
        $this->db->exec("CREATE TABLE IF NOT EXISTS mod_zarinpal (id int(11) NOT NULL AUTO_INCREMENT,
    ...
    {
        $this->db->exec('DROP TABLE IF EXISTS mod_zarinpal');
        return true;
    }
    ...
    public function drawForm()
    {
        ...
        $query = $this->db->prepare("SELECT * FROM mod_zarinpal WHERE invoice_id = ? AND NOT status = 'Failed'");
        ...
        if($count > 0)
        {
            ...
            if($session['status'] == 'Pending')
            {
                ...
                $query = $this->db->prepare("UPDATE mod_zarinpal SET amount = ?, toman = ?, conversion_rate = ?, remittance_fees = ?, order_id = ? WHERE id = ?");
        ...
        if($form == null)
        {
            if($session_id == null)
            {
                $query = $this->db->prepare("INSERT INTO mod_zarinpal (client_id, invoice_id, amount, toman, conversion_rate, remittance_fees, order_id, status) VALUES(?, ?, ?, ?, ?, ?, ?, 'Pending')");
    ...
    function callback()
    {
        ...
        {
            $query = $this->db->prepare("SELECT * FROM mod_zarinpal WHERE authority = ?");
                    ...
                    if($res['Status'] == 100)
                    {
                        ...
                        if($count > 0)
                        {
                            ...
                            // {
								...
								$query = $this->db->prepare("UPDATE mod_zarinpal SET payment_datetime = NOW(), status = 'Paid' WHERE authority = ?");
                                $query->execute(array($Authority));
                                $query->closeCursor();
        ...
        if($error != null)
        {
			...
			$query = $this->db->prepare("UPDATE mod_zarinpal SET payment_datetime = NOW(), status = ? WHERE authority = ?");
...
<head>
    ...
    <style type="text/css">
        ...
        #icon {
            width: 128px;
            height: 128px;
            background-image: url(/includes/modules/Payment/zarinpal/' . $icon_name . ');
            margin: 15px auto;
        }
        hr {
            border: 1px solid #cccccc;
        ...
        .button {
            ...
            behavior: url(/includes/modules/Payment/zarinpal/border-radius.htc);
            border-radius: 10px;
